...
import { unzip_upload } from "../../../../../utils/dataHandle";
...
const Unzip = () => {
  ...
  const process = async (e) => {
    ...
    if (file) {
      const res = await unzip_upload(file);
      setZStatus({
        tick: res ? "bg-green-300" : "bg-red-300",
        filename: file.name,
      });
    }
  };
  ...
  return (
    <div className="flex-between h-full w-full gap-3 px-1 2xl:gap-5 2xl:px-3">
      <div className="flex-start h-14 w-60 2xl:h-16 2xl:w-56">
        <UploadCont upload_info={upload_info} />
      </div>
      <div className="w-full 2xl:text-xl 2xl:font-semibold">
        <div className="mx-auto">{zstatus.filename}</div>
      </div>
      <div
