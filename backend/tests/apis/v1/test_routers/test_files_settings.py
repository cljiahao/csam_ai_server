...
def mock_update_settings(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    mock = MagicMock()
    monkeypatch.setattr("apis.v1.routers.files.update_settings", mock)
    return mock
...
def test_upload_settings_success(
    ...
    response = test_client.post(f"/v1/upload/settings", files=file_data)
...
def test_upload_settings_invalid(
    ...
    response = test_client.post(f"/v1/upload/settings", files={})
